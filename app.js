...
// Routes
app.get("/", (req, res) => {
  req.flash("success", "Home page me aa gai aap");
...
app.post("/rescue", upload.single('file'), async (req, res) => {
  try {
    ...
    const { name, email, phoneNumber, radius , latitude , longitude} = req.body;
    const newRescue = new rescueuser({ name, email, phoneNumber, radius , latitude , longitude});
